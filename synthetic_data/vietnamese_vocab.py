...
This module enables generation of 100K+ unique product names for VLM training.
Vocabulary is loaded from JSON files for easy maintenance and extension.
"""
...
class ProductCategory(Enum):
    ...
    HOUSEHOLD = "Gia dụng"
    ELECTRONICS = "Điện tử"
    CLOTHING = "Phần cứng"  # Mapping hardware/clothing together
    PHARMACY = "Thuốc"
...
FOOTER_MESSAGES = _common_data.get("footer_messages", [])
TABLE_UNITS = _common_data.get("table_units", [])
...
class VietnameseVocabGenerator:
    ...
    def generate_unique_product_name(self, max_attempts: int = 10, **kwargs) -> str:
        """Generate a unique product name not seen before."""
    ...
    def generate_product(self, category: ProductCategory = None) -> GeneratedProduct:
        """Generate a complete product with name, price, and metadata."""
    ...
    def random_vietnamese_text(self, word_count: int = 5) -> str:
        """Generate random Vietnamese text for testing."""
        words = []
        adjectives = ADJECTIVES if ADJECTIVES else ["sản phẩm"]
        numbers = list(VIETNAMESE_NUMBERS.get("cardinal", {1: "một"}).values())
        units = UNITS if UNITS else ["cái"]
        store_names = STORE_NAMES_VIETNAMESE if STORE_NAMES_VIETNAMESE else ["Cửa hàng"]
...
def generate_vietnamese_products(count: int = 100) -> List[GeneratedProduct]:
    """Quick function to generate Vietnamese products."""
    generator = VietnameseVocabGenerator()
